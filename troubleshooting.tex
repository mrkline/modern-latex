...
begins the next.
\LaTeX{} makes some effort to avoid these, but unfortunately, the algorithm it
...
`local' rather than `global' optimization.\quotekern''\,\endnote{Ibid, 110}}
You can try to increase the ``penalty'' for doing so in order to make \LaTeX{}
try harder to avoid it with:\footnote{When considering a given layout,
\LaTeX{} assigns penalties, or ``badness''\quotekern,
to anything that arguably makes a document look worse.
Examples include stretching or shrinking spacing, or leaving orphans and widows.
It then chooses the whichever layout it generated that has the least badness.}
\begin{leftfigure}
\begin{lstlisting}
